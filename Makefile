...
#   make test-poly  run tests under Poly/ML (via tools/polybuild)
#   make all-tests  run the suite under both compilers
#   make clean      remove build artifacts
MLTON      ?= mlton
BIN        := bin
LIBDIR     := lib/github.com/sjqtentacles/sml-toml
VENDOR     := lib/github.com/sjqtentacles/sml-parsec
TEST_MLB   := test/sources.mlb
SRCS       := $(wildcard $(LIBDIR)/*.sml $(LIBDIR)/*.sig $(VENDOR)/*.sml $(VENDOR)/*.sig) \
              $(wildcard test/*.sml) $(TEST_MLB) $(LIBDIR)/sources.mlb $(VENDOR)/parsec.mlb
